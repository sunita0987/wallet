  ...
  return (
    <div className="relative w-80 rounded-4xl overflow-hidden shadow-lg mt-10 ml-10">
      ...
      <div className="absolute bottom-0 w-full bg-gradient-to-t from-black/70 to-transparent text-white flex justify-around py-4">
        <button onClick={() => setLikes(likes + 1)} className="flex items-center space-x-1 cursor-pointer">
          <FaHeart className="text-white" />
          <span>{likes}</span>
        </button>
        <button onClick={() => setComments(comments + 1)} className="flex items-center space-x-1 cursor-pointer">
